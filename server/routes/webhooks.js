...
function collectStringCandidates(value, out) {
  ...
  if (Array.isArray(value)) { for (const v of value) collectStringCandidates(v, out); return; }
  if (typeof value === 'object') { for (const v of Object.values(value)) collectStringCandidates(v, out); }
}
...
function eventMatchesFilters(ev, filters) {
  ...
  function hasToken(token) {
    ...
    for (const cand of normalized) { if (cand.includes(t)) return true; }
    return false;
  }
...
router.post('/receive', (req, res) => {
  ...
  if (webhookEvents.length > 20) webhookEvents = webhookEvents.slice(0, 20);
...
router.get('/events', (req, res) => {
  ...
  res.json(webhookEvents.filter((ev) =>
...
router.get('/stream', (req, res) => {
  ...
  const client = { res, filters: { trackingNumber, labelId, shipmentId, resourceId } };
